...
%Bus1: Slack -> #28(only)[0.9583,-0.2323], #48 (only) [0.9753,-0.3584], #28(with 48)[0.9572,-0.2620], #48(with 28)[0.9706, -0.4006]
bus_a = bus.slack(0.9753,-0.3584,shunt);
...
%% simul(fault)
out_baseNet_68BS = baseNet_68BS.simulate([0,10],'fault',{{[1,1.01],48}});
out_baseNet_68BS.plot('para', 'omega')

%% Expand 68BS based on Test 1
% original Bus 48: P=-0.2412, Q=-2.2000, Vabs=1.0337, Vangle=-0.3207

exPand_Bus48 = network.IEEE68bus();
...
%Bus69: PQ Load
bus_b = bus.PQ(-0.217,-0.127,shunt);
exPand_Bus48.add_bus(bus_b);

%Bus70: PQ Load
bus_c = bus.PQ(-0.942,-0.19,shunt);
% bus_3 = bus.PV_2comp(0,1.01,[0.942,0.19],shunt);
exPand_Bus48.add_bus(bus_c);

%Bus71: PQ Load
bus_d = bus.PQ(-0.478,0.039,shunt);
exPand_Bus48.add_bus(bus_d);

%Bus72: PQ Load
bus_e = bus.PQ(-0.076,-0.016,shunt);
% bus_5 = bus.PQ(0.076,0.016,shunt);
exPand_Bus48.add_bus(bus_e);

%Bus73: PQ Load
bus_f = bus.PQ(-0.112,-0.075,shunt);
exPand_Bus48.add_bus(bus_f);

%Bus74: Transformer
bus_g = bus.PQ(0,0,shunt);
exPand_Bus48.add_bus(bus_g);

%Bus75 Zero PQ Load
bus_h = bus.PQ(0,0,shunt);
exPand_Bus48.add_bus(bus_h);

%Bus76: PQ Load
bus_i = bus.PQ(-0.295,-0.166,[0, 0.19]);
% bus_9 = bus.PQ(0.295,0.166,[0, 0.19]);
exPand_Bus48.add_bus(bus_i);

%Bus77: PQ Load
bus_j = bus.PQ(-0.09,-0.058,shunt);
% bus_9 = bus.PQ(0.09,0.058,shunt);
exPand_Bus48.add_bus(bus_j);

%Bus78: PQ Load
bus_k = bus.PQ(-0.035,-0.018,shunt);
% bus_10 = bus.PQ(0.035,0.018,shunt);
exPand_Bus48.add_bus(bus_k);

%Bus79: PQ Load
bus_l = bus.PQ(-0.061,-0.016,shunt);
% bus_11 = bus.PQ(0.061,0.016,shunt);
exPand_Bus48.add_bus(bus_l);

%Bus80: PQ Load
bus_m = bus.PQ(-0.135,-0.058,shunt);
% bus_12 = bus.PQ(0.135,0.058,shunt);
exPand_Bus48.add_bus(bus_m);

%Bus81: PQ Load
bus_n= bus.PQ(-0.149,-0.05,shunt);
% bus_13 = bus.PQ(0.149,0.05,shunt);
exPand_Bus48.add_bus(bus_n);

% Branch defination
%Branch 48 to 69
branch_ab = branch.pi(48,69,[0.01938, 0.05917], 0.0528/2);
exPand_Bus48.add_branch(branch_ab);

%Branch 48 to 72
branch_ae = branch.pi(48,72,[0.05403, 0.22304], 0.0492/2);
exPand_Bus48.add_branch(branch_ae);

%Branch 69 to 70
branch_bc = branch.pi(69,70,[0.04699, 0.19797], 0.0438/2);
exPand_Bus48.add_branch(branch_bc);

%Branch 69 to 71
branch_bd = branch.pi(69,71,[0.05811, 0.17632], 0.034/2);
exPand_Bus48.add_branch(branch_bd);

%Branch 69 to 72
branch_be = branch.pi(69,72,[0.05695, 0.17388], 0.0346/2);
exPand_Bus48.add_branch(branch_be);

%Branch 70 to 71
branch_cd = branch.pi(70,71,[0.06701, 0.17103], 0.0128/2);
exPand_Bus48.add_branch(branch_cd);

%Branch 71 to 72
branch_de = branch.pi(71,72,[0.01335, 0.04211], 0);
exPand_Bus48.add_branch(branch_de);

%Branch 71 to 74
branch_dg = branch.pi_transformer(71,74,[0, 0.20912],0,0.978,0);
exPand_Bus48.add_branch(branch_dg);

%Branch 71 to 76
branch_di = branch.pi_transformer(71,76,[0, 0.55618],0,0.969,0);
exPand_Bus48.add_branch(branch_di);

%Branch 72 to 73
branch_ef = branch.pi_transformer(72,73,[0, 0.25202],0,0.932,0);
exPand_Bus48.add_branch(branch_ef);

%Branch 73 to 78
branch_fk = branch.pi(73,78,[0.09498, 0.1989],0);
exPand_Bus48.add_branch(branch_fk);

%Branch 73 to 79
branch_fl = branch.pi(73,79,[0.12291, 0.25581],0);
exPand_Bus48.add_branch(branch_fl);

%Branch 73 to 80
branch_fm = branch.pi(73,80,[0.06615, 0.13027],0);
exPand_Bus48.add_branch(branch_fm);

%Branch 74 to 75
branch_gh = branch.pi_transformer(74,75,[0, 0.17615],0,1,0);
exPand_Bus48.add_branch(branch_gh);

%Branch 74 to 76
branch_gi = branch.pi_transformer(74,76,[0, 0.11001],0,1,0);
exPand_Bus48.add_branch(branch_gi);

%Branch 76 to 77
branch_ik = branch.pi(76,77,[0.03181, 0.0845],0);
exPand_Bus48.add_branch(branch_ik);

%Branch 76 to 81
branch_in = branch.pi(76,81,[0.12711, 0.27038],0);
exPand_Bus48.add_branch(branch_in);

%Branch 77 to 78
branch_jk = branch.pi(77,78,[0.08205, 0.19207],0);
exPand_Bus48.add_branch(branch_jk);

%Branch 79 to 80
branch_lm = branch.pi(79,80,[0.22092, 0.19988],0);
exPand_Bus48.add_branch(branch_lm);

%Branch 80 to 81
branch_mn = branch.pi(80,81,[0.17093, 0.34802],0);
exPand_Bus48.add_branch(branch_mn);
...
%Bus2 Load
component_b = component.load.impedance();
exPand_Bus48.a_bus{69}.set_component(component_b);
...
%Bus3 Load
component_c = component.load.impedance();
exPand_Bus48.a_bus{70}.set_component(component_c);

%Bus4 load
component_d = component.load.impedance();
exPand_Bus48.a_bus{71}.set_component(component_d);

%Bus5 load
component_e = component.load.impedance();
exPand_Bus48.a_bus{72}.set_component(component_e);
...
%Bus6 load
component_f = component.load.impedance();
exPand_Bus48.a_bus{73}.set_component(component_f);

%Bus7 load
% component_g = component.load.impedance();
% exPand_Bus48.a_bus{74}.set_component(component_g);
...
%Bus8 load
% component_h = component.load.impedance();
% exPand_Bus48.a_bus{75}.set_component(component_h);

%Bus9 load
component_i = component.load.impedance();
exPand_Bus48.a_bus{76}.set_component(component_i);

%Bus10 load
component_j = component.load.impedance();
exPand_Bus48.a_bus{77}.set_component(component_j);

%Bus11 load
component_k = component.load.impedance();
exPand_Bus48.a_bus{78}.set_component(component_k);

%Bus12 load
component_l= component.load.impedance();
exPand_Bus48.a_bus{79}.set_component(component_l);

%Bus13 load 
component_m = component.load.impedance();
exPand_Bus48.a_bus{80}.set_component(component_m);

%Bus14 load 
component_n = component.load.impedance();
exPand_Bus48.a_bus{81}.set_component(component_n);

%%%%%%%%%% save model for exPand_Bus28and48 %%%%%%%%%%
exPand_Bus28and48 = exPand_Bus48;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


exPand_Bus48.initialize;
powerflow_exPand_Bus48 = (exPand_Bus48.V_equilibrium).*conj(exPand_Bus48.I_equilibrium);
...
%check active power
disp('------ACTIVE POWER[MW]------')
disp(real(powerflow_exPand_Bus48)*100)
disp('----------------------------')

%check active power
disp('------REACTIVE POWER[MVAR]------')
disp(imag(powerflow_exPand_Bus48)*100)
disp('----------------------------')

%check power loss
disp('------POWER LOSS[MVA]------')
disp(sum(powerflow_exPand_Bus48)*100)
disp('----------------------------')

%check bus voltage
disp('------BUS VOLTAGE[p.u.]------')
disp(abs(exPand_Bus48.V_equilibrium))
disp('----------------------------')

%check bus voltage' phase
disp('------BUS VOLTAGE PHASE[rad]------')
disp(angle(exPand_Bus48.V_equilibrium))
disp('----------------------------')

%check bus voltage' phase
disp('------BUS VOLTAGE PHASE[degree]------')
disp(rad2deg(angle(exPand_Bus48.V_equilibrium)))
disp('----------------------------')

%% simul(fault)
% out_exPand_Bus48 = exPand_Bus48.simulate([0,15],'fault',{{[1,1.01],81},{[3,3.01],73},{[5,5.01],70},{[9,9.01],76}});
out_exPand_Bus48 = exPand_Bus48.simulate([0,10],'fault',{{[1,1.01],48}});
% out_exPand_Bus48 = exPand_Bus48.simulate([0,10],'fault',{{[1,1.01],69:81}});
out_exPand_Bus48.plot('para', 'omega')